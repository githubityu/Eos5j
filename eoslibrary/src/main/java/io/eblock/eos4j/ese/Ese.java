...
//		 }
	...
	public static String parseunDelegateData(String from, String receiver, String stakeNetQuantity,
		...
		DataParam[] datas = new DataParam[] { new DataParam(from, DataType.name, Action.undelegatebw),
					new DataParam(receiver, DataType.name, Action.undelegatebw),
					new DataParam(stakeNetQuantity, DataType.asset, Action.undelegatebw),
					new DataParam(stakeCpuQuantity, DataType.asset, Action.undelegatebw),
			};
	...
	public static String parseSellRamData(String seller, Long bytes) {

		DataParam[] datas = new DataParam[] { new DataParam(seller, DataType.name, Action.ram),
	...
	public static String parseLinkAuth(String account, String code, String type, String requirement) {
		DataParam[] datas = new DataParam[]{
				new DataParam(account, DataType.name, Action.LINK_AUTH),
				new DataParam(code, DataType.name, Action.LINK_AUTH),
				new DataParam(type, DataType.name, Action.LINK_AUTH),
				new DataParam(requirement, DataType.string, Action.LINK_AUTH),
		};
		return bytesToHexString(datas);
	}

	private static String bytesToHexString(DataParam[] datas) {
		byte[] allbyte = new byte[]{};
		for (DataParam value : datas) {
			allbyte = ByteUtils.concat(allbyte, value.seria());
		}
